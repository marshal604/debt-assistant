...
export class GroupService {
  createGroup$(item: GroupItem): Promise<void> {
    return HttpClient.middle(Firebase.db.collection('group').add(item)).then(ref => ref.set({ id: ref.id }, { merge: true }));
  }
  ...
  getGroupLendCurrency(userId: string, groupId: string): Promise<number> {
    return HttpClient.middle(this.getGroupLendItems$(userId, groupId).then(data => data.reduce((pre, cur) => pre + +cur.currency, 0)));
  }
  ...
  getGroupDebtCurrency(userId: string, groupId: string): Promise<number> {
    return HttpClient.middle(
      this.getGroupDebtItems$(userId, groupId).then(data => data.reduce((pre, cur) => pre + +(cur.currency / cur.debtorIds.length), 0))
    );
  }
  ...
  batchCreateGroupDetail$(groupId: string, details: GroupDetailItem[]): Promise<void> {
    return HttpClient.middle(Promise.all(details.map(detail => this.createGroupDetail$(groupId, detail))).then());
  }
  ...
  batchDeleteGroupTemplate$(groupId: string, templateIds: string[]): Promise<void> {
    return HttpClient.middle(Promise.all(templateIds.map(id => this.deleteGroupTemplate$(groupId, id))).then());
  }
}
